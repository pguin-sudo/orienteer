...
def get_formatted_timedelta(timedelta: timedelta) -> str:
    ...
    hours_string = '' if hours == 0 else f', {
        int(hours)} ч.' if days > 0 else f'{int(hours)} ч.'
    min_string = '' if minutes == 0 else f', {
        int(minutes)} мин.' if hours > 0 else f'{int(minutes)} мин.'
