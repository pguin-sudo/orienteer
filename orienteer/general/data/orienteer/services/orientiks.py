...
async def _init_balance(user_id: UUID) -> None:
    ...
    async with database_helper.session_factory() as db_session:
        await orientiks.add_time_balancing(
            db_session, user_id, int(overall.total_seconds() // 3600 * PRICE_FOR_INIT)
        )
    ...
    if db_session is None:
        async with database_helper.session_factory() as db_session:
            ...
            if raw_info is None:
                ...
                raw_info = await orientiks.get_balance_raw_info(
...
async def do_transfer(
    ...
    async with database_helper.session_factory() as db_session:
        ...
        await orientiks.add_orientiks_from_friends(
            db_session, recipient_user_id, amount
        )
