...
async def get_last_purchase_of_product(db_session: AsyncSession, user_id: UUID, product_id: int):
    stmt = (select(Purchase).filter(user_id == Purchase.user_id).filter(product_id == Purchase.product_id).order_by(
        desc(Purchase.date)).limit(1))
...
async def get_all_purchases_of_user(db_session: AsyncSession, user_id: UUID) -> tuple[Purchase, ...]:
    result = await db_session.execute(
        select(Purchase).filter(user_id == Purchase.user_id).order_by(desc(Purchase.date)))
    return tuple(result.scalars().all())
