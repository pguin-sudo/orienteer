...
def plot_orientiks_cached_info(all_info: tuple[OrientiksCachedInfo, ...]) -> str:
    ...
    with plt.style.context('cyberpunk'):
        ...
        plt.plot(df['date'], df['total_sponsorship'], label='Спонсорские', color=(1, 1, 0.3, 1), lw=1, marker='', ms=20)
        plt.plot(df['date'], df['total_spent'], label='Потраченные', color=(1, 0.3, 1, 1), lw=1, marker='', ms=20)
        plt.plot(df['date'], -df['total_fine'], label='Штрафные', color=(1, 0.3, 0.3, 1), lw=1, marker='', ms=20)
        plt.plot(df['date'], df['total_from_time'] - df['total_time_balancing'], label='За наигранное время',
                 color=(0.3, 1, 0.3, 1), lw=1, marker='', ms=20)

        plt.plot(df['date'],
                 df['total_from_time'] - df['total_time_balancing'] - df['total_fine'] + df['total_sponsorship'],
                 label='Общее', color=(1, 1, 1, 1), lw=2, marker='', ms=20)
