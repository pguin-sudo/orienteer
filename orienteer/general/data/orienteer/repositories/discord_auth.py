...
async def link_discord(
    db_session: AsyncSession, user_id: UUID, discord_user_id: int, discord_username: str
) -> None:
...
async def get_discord_user_id_by_user_id(
...
async def get_user_id_by_discord_user_id(
    db_session: AsyncSession, discord_user_id: int
) -> UUID | None:
    query = select(DiscordAuth.user_id).filter(
        discord_user_id == DiscordAuth.discord_user_id
    )
    result = await db_session.execute(query)
...
async def get_all_authorized(db_session: AsyncSession) -> tuple[DiscordAuth, ...]:
    result = await db_session.execute(
        select(DiscordAuth).order_by(desc(DiscordAuth.created_at))
    )
