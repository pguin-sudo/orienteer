...
from orienteer.general.data.ss14.services import playtime, seen_time, bans
...
async def check_elderings():
    for auth_data in await discord_auth.get_all_authorized():
        ...
        if (
            play_time > timedelta(days=14)
            and datetime.now(timezone.utc)
            - await seen_time.get_first_seen_time(auth_data.user_id)
            > timedelta(days=31)
            and (await bans.get_fine(auth_data.user_id)) < 300
        ):
