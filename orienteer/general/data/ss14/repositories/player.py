...
async def get_user_id(ckey: str) -> UUID | None:
    async with DBConnectionContextManager() as connection:
        result = await connection.fetchval("SELECT user_id FROM player WHERE last_seen_user_name = $1", ckey)
...
async def get_user_id_nocased(ckey: str) -> tuple[UUID | None, str]:
    async with DBConnectionContextManager() as connection:
        players = await connection.fetch(
            "SELECT user_id, last_seen_user_name FROM player WHERE LOWER(last_seen_user_name) = LOWER($1)", ckey)
