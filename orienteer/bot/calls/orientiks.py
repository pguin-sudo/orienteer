...
from orienteer.general.data.orienteer.services import (transactions, purchases, )
...
class Balance(AbstractCall):
    async def __call__(self, user_dto: UserDTO) -> None:
        await self.interaction.edit_original_message(embed=embeds.result_message(title=f"Баланс {user_dto.ckey}:",
            content=f"{await transactions.get_balance(user_dto.user_id)} {CURRENCY_SIGN}", ))


class Transfer(AbstractCall):
    async def __call__(self, sender_user_dto: UserDTO, recipient_user_dto: UserDTO, amount: int) -> None:
        amount = int(amount)
        if amount <= 0:
        ...
        await transactions.do_transfer(sender_user_dto.user_id, recipient_user_dto.user_id, amount)
    ...
    async def __call__(self, user_dto: UserDTO) -> None:
        ...
        def create_callback(product_: AbstractProduct):
            async def buy(interaction: Interaction):
                responding_user_dto = await UserDTO.from_discord_user_id(interaction.user.id)
                ...
                purchase_cooldown = await purchases.get_purchase_cooldown(responding_user_dto.user_id, product_)
                ...
                await purchases.create_purchase(responding_user_dto.user_id, product_.id, price_)
        ...
        for product in products_array:
            if (not (await product.can_buy(user_dto.user_id)) or await purchases.get_purchase_cooldown(user_dto.user_id,
                                                                                              product) is not None):
                continue
...
class Purchases(AbstractCall):
    async def __call__(self, user_dto: UserDTO):
        ...
        if user_purchases is None or user_purchases == ():
            await self.interaction.edit_original_message(
                embed=embeds.result_message(f"Покупки {user_dto.ckey}:", content="Отсутствуют"))
            return
    ...
    async def __call__(self, amount: int, user_dto: UserDTO) -> None:
        ...
        async def pay_callback(interaction: Interaction):
            ...
            await self.interaction.edit_original_message(embed=embeds.success_message(title=Success.transfer.value,
                content=f"Вы приобрели {amount}{CURRENCY_SIGN} за {int(buy_price * amount)}₽", ), view=button_view, )
