...
def get_formatted_timedelta(timedelta_: timedelta) -> str:
    ...
    days, hours, minutes = (
        timedelta_.days,
        timedelta_.seconds // 3600,
        timedelta_.seconds % 3600 // 60,
    )
    days_string = "" if days == 0 else f"{int(days)} д."
    hours_string = (
        "" if hours == 0 else f", {int(hours)} ч." if days > 0 else f"{int(hours)} ч."
    )
    min_string = (
        ""
        if minutes == 0
        else f", {int(minutes)} мин." if hours > 0 else f"{int(minutes)} мин."
    )
