...
async def get_balance_raw_info(db_session: AsyncSession, user_id: UUID) -> Orientiks | None:
...
async def add_time_balancing(db_session: AsyncSession, user_id: UUID, time_balancing: int) -> None:
    result = await db_session.execute(select(Orientiks).where(user_id == Orientiks.user_id))
    ...
    if record:
        ...
        await db_session.execute(
            update(Orientiks).where(user_id == Orientiks.user_id).values(time_balancing=new_time_balancing))
    else:
        await db_session.execute(insert(Orientiks).values(user_id=user_id, time_balancing=time_balancing))

    await db_session.commit()


async def add_orientiks_from_friends(db_session: AsyncSession, user_id: UUID, amount: int) -> None:
    result = await db_session.execute(select(Orientiks).where(user_id == Orientiks.user_id))
    record = result.scalars().first()
    ...
    if record:
        ...
        await db_session.execute(
            update(Orientiks).where(user_id == Orientiks.user_id).values(friends=new_friends_amount))
    else:
        await db_session.execute(insert(Orientiks).values(user_id=user_id, friends=amount))
...
async def add_spent(db_session: AsyncSession, user_id: UUID, amount: int) -> None:
    ...
    await db_session.execute(update(Orientiks).where(user_id == Orientiks.user_id).values(spent=new_spent))
    await db_session.commit()
