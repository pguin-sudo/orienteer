...
def _have_privileges(sponsor: Sponsor) -> bool:
    if (
        sponsor.extra_slots == 0
        and not sponsor.ooc_color
        and not sponsor.allowed_markings
        and not sponsor.ghost_theme
        and not sponsor.sponsor_chat
        and not sponsor.priority_join
    ):
        return False
    else:
        return True
...
async def add_extra_clots(user_id: UUID, amount: int) -> Sponsor | None:
    async with database_helper.session_factory() as db_session:
        ...
        return await sponsors.update_sponsor(
            db_session, user_id, extra_slots=sponsor.extra_slots + amount
        )
...
async def set_sponsor_chat(user_id: UUID, status: bool):
    async with database_helper.session_factory() as db_session:
        await sponsors.try_create_empty_sponsor(db_session, user_id)
        discord_user_id = await discord_auth.get_discord_user_id_by_user_id(
            db_session, user_id
        )
