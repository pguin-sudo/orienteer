...
async def get_last_sent_role_ban_id(db_session: AsyncSession) -> int:
    result = await db_session.execute(
        select(SentBan.last_sent_role_ban_id).order_by(
...
async def set_last_sent_role_ban_id(db_session: AsyncSession, id: int) -> None:
    await db_session.execute(
        update(SentBan).values(last_sent_role_ban_id=id)
    )
    await db_session.commit()
