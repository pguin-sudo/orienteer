...
async def check_bans():
    ...
    for ban in new_bans:
        ...
        admin_name = (
            await player.get_ckey(banning_admin) if banning_admin else "Неизвестно"
        )
        player_name = (
            await player.get_ckey(player_user_id) if player_user_id else "Неизвестно"
        )
        ...
        embed_desc += f"**Штраф:** {fine} {CURRENCY_SIGN}\n\n"
        embed_desc += f"**Причина:** {reason}\n"

        await send_discord_message(
            WEBHOOKS_BANS, USERNAME, embed_title, embed_desc, color
        )
